...
\begin{frame}{No todo se detecta mejor con IA}
  ...
  \item \textbf{SmartUniversity} IoT de la UA que captura diferentes parámetros de afluencia y temporal.
  \end{itemize}
\end{frame}
...
\begin{frame}{¿Qué tenemos a disposición?}
  ...
  \item Procesamiento centralizado (SPOF). ¿vigilancia urbanización?
...
\begin{frame}{Ejecución en \textit{hardware} limitado}
  ...
  \end{table}
  \vspace{0.2cm} Es importante la \textbf{cuantización.} Para correr en ESP32, los modelos se comprimen de \texttt{float32} a \texttt{int8}, reduciendo el tamaño del modelo de 50MB a $<500$KB.
...
\section{Pizarra++}
\begin{frame}{Ideas para apuntar}
  ...
  \begin{enumerate}
  \item Facilitar la \textbf{grabación y \textit{streaming}} de clases.
  \item \textbf{Seguimiento de movimiento} del profesor para el punto anterior.
  \item Guardar un \textbf{historial} del trazado de la pizarra.
  \item \textbf{Transcripciones de voz} de la lección.
  \item \textbf{Accesibilidad} (intérpretes, traductores...). 
  \end{enumerate}
\end{frame}

\begin{frame}{Experiencias previas}
  \begin{enumerate}
  \item Durante la pandemia del \textbf{covid}:
    \begin{itemize}
    \item Las reuniones a través de videollamadas.
    \item Poca preparación, mal audio, aplicaciones propietarias (no FOSS).
    \item Compatir pantalla, pizarra con mala visibiidad.
    \item Profesores pueden emplear otras para difundir conocimiento: \url{https://youtu.be/_KNxz2YyhgA}. 
    \end{itemize}
  \item El seguimiento del movimiento del profesor puede hacerse de \textbf{forma automática} desde la configuración de algunas cámaras.
    \begin{itemize}
    \item \url{https://youtu.be/ZA-tUyM_y7s&t=120}.
    \item Aunque siempre se deba contemplar una opción manual.
    \end{itemize}
  \item \textbf{Guardar algo escrito de la pizarra}.
    \begin{itemize}
    \item Fotos a la pizarra $\rightarrow$ mala calidad al detalle $\land$ rostros no deseados.
    \item Materiales docentes, almacenamiento en la \enquote{nube}...
    \end{itemize}
  \item A partir de los vídeos \textbf{es posible extraer una transcripción}.
  \item Garantizar \textbf{accesibilidad} para personas ciegas (braille), sordas de lo acontecido en la sección (generar intérpretes).
  \end{enumerate}
\end{frame}
...
\begin{frame}{Consideraciones}
  El sistema debe:
  \begin{itemize}
  \item Estar \textbf{a merced del profesor} como una herramienta. Y es este quién decida cuando hacerla servir.
  \item Garantizar la \textbf{privacidad} (GDPR). Tratamiendo de datos en \textit{edge} o en local.
  \item Fácil mecanismo para \textbf{cambiar parámetros}: \textit{ON} $\leftrightarrow$ \textit{OFF}, historial sí $\leftrightarrow$ no, movimiento sí $\leftrightarrow$ no... 
  \item \textbf{Independiente} al tipo de pizarra (da igual si es de tiza o digital).
  \item Tener la posibilidad de guardar (\textbf{exportación}) el vídeo/traza de la pizarra en fichero.
  \item ¿Tareas paralelas? (Grabar y sacar grabados de la pizarra).
  \end{itemize}
\end{frame}

\begin{frame}{Emisiones en vivo. Grabaciones.}
  ...
  \begin{itemize}
  \item Tener un \textbf{mando IR} o \textbf{algo físico} que indique qué hacer a la cámara. (opción con limitaciones)
  \item \textbf{Aplicación web} en la que el profesor decida los parámetros.
  \end{itemize}
...
\subsection{\textit{Software} para implementar}
\begin{frame}{¿Cuándo capturar trazas?}  
  ...
  \begin{columns}
    \column{0.4840829572\textwidth}
    \begin{block}{Disparador}
      La captura se activa mediante un \textbf{botón físico} solicitado por el profesor al terminar una explicación.
    \end{block}
    
    \column{0.4840829572\textwidth}
    \begin{alertblock}{Detección automática}
      Mediante \textbf{\textit{background subtraction}}, el \textit{software} detecta cuando la pizarra está \enquote{llena} y el profesor se aparta, disparando la foto automáticamente.
    \end{alertblock}
  \end{columns}

  \begin{itemize}
  \item Demo de \textit{background substraction} con OpenCV: \url{https://docs.opencv.org/4.x/d1/dc5/tutorial_background_subtraction.html}.
  \item \textbf{Cámara PTZ con \textit{auto-tracking}}: \url{https://github.com/AutoPTZ/autoptz}
  \end{itemize}
\end{frame}

\begin{frame}{Privacidad y procesamiento}
  \textbf{Proceso de anonimización:}
  \begin{enumerate}
  ...
  \item \textbf{Aplicación de difuminado:} Gaussiana sobre las regiones de interés (ROIs) detectadas.
  \item \textbf{Rectificación de la perspectiva:} Algoritmo de homografía para transformar la foto lateral en una imagen plana escaneada.
  \item \textbf{Mejora de contraste:} Binarización adaptativa para convertir trazos de tiza/rotulador en digital nítido.
  \item \textbf{Extracción en texto:} Bien uitilizar OCR para sacar texto de imagen o Whisper (OpenAI) para el audio $\rightarrow$ transcripción y subtítulos.
  \item \textbf{Exportación a múltiples formatos:} Bien entre documentos (pandoc) o entre vídeo (ffmpeg). 
  \end{enumerate}
\end{frame}
...
\begin{frame}{Enlaces de interés}
  ...
  \item \textbf{Whisper:} \url{https://github.com/openai/whisper}.
  \item \textbf{Tesseract OCR:} \url{https://github.com/tesseract-ocr/tesseract}.
  \item \textbf{MTCNN:} \url{https://github.com/ipazc/mtcnn}.
  \end{itemize}
\end{frame}
\end{document}
